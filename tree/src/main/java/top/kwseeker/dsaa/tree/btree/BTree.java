...
public class BTree<K extends Comparable, V> {
    ...
    private final Stack<StackInfo> mStackTracer = new Stack<>();
    ...
    // 节点key满了分裂
    // 节点分裂时从中间分裂成两个新节点，中间的
    // parentNode 是新建的节点，nodeIdx是指要分裂的节点btNode作为parentNode的第几个子节点
    ...
    //
    public void list(BTreeIterator<K, V> iterImpl) {
        if (mSize < 1) {
            return;
        }
    ...
    // Return false if it is done
    //
    private boolean listEntriesInOrder(BTNode<K, V> treeNode, BTreeIterator<K, V> iterImpl) {
        if ((treeNode == null) ||
            ...
            if ((leftSibling != null) && (leftSibling.mCurrentKeyNum > BTNode.LOWER_BOUND_KEYNUM)) {
                ...
                if ((rightSibling != null) && (rightSibling.mCurrentKeyNum > BTNode.LOWER_BOUND_KEYNUM)) {
                    ...
                    if (isRebalanceNeeded && (mRoot != null)) {
                        rebalanceTree(mRoot, parentNode, (K) parentNode.mKeys[0].mKey);
                    }
                }
            }
     ...
     */
    static class StackInfo<K extends Comparable, V> {
    ...
    public static class BTreeIterator<K extends Comparable, V> {
